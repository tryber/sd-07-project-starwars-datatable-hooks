...
function Table() {
  const { data, pageLoading, setpageLoading, headers, filters } = useContext(StarWarsContext);
  ...
  return (
    <div>
      ...
      <table>
        <tr>
          {pageLoading ? null : headers.map((tHead) => (
            <th key={ tHead }>{tHead}</th>
          ))}
        </tr>
        <tbody>
          {pageLoading ? 'Loading' : data.filter((planets) => filterOptions(planets))
